...
3. Click **Build query** — the generated LogParser query appears in an editable text box (adjust it
   manually if needed, e.g. for conditions more advanced than the GUI covers).
4. Choose whether the result goes to the **Screen** (table inside the app) or to a **File** (CSV).
5. Click **Run**.
...
### The "Account statistics (Stats)" tab
...
2. Optionally filter on account name, and tick "Include groups and server total" if you also want
   the aggregated rows (`VirtGroup`/`ServerTotal`) and not just individual accounts.
...
The app's **Copy query** button copies the query the app would run, so you can paste it straight
into `LogParser.exe` from Windows Terminal (PowerShell) if you'd rather run it yourself or debug a
query.
...
- `-o:CSV` gives the same table format as the app's result view. Switch to `-o:DATAGRID` for an
  ...
  exactly this when you pick **File** as the output — there is no `-o:CSV:file` flag despite how
  natural it looks (tested and confirmed wrong: gives `Unknown output format`).
- If you ticked **several** event types in the GUI, the copied text contains several blocks, one
  per category, separated by a heading line `### Category`. Remove the heading line and run each
...
Concrete example — the Stats tab (account overview), verified against real Stats files:

```powershell
LogParser.exe -q -i:XML -fNames:XPath -fMode:Tree -o:CSV "SELECT DISTINCT /stats/@type AS Type, /stats/@account AS Account, /stats/info/@lastLogin AS LastLogin, /stats/total/data/@loginCount AS LoginCount, /stats/total/data/@bytesReceived AS BytesReceived, /stats/total/data/@bytesSent AS BytesSent FROM 'C:\Stats\*.xml' WHERE Type = 'VirtAccount' ORDER BY LastLogin DESC"
```
